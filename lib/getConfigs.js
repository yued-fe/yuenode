...
module.exports = {
    ...
    getOriginRouterMap(){
        ...
        try {
            ...
            // 没有的话空转
            console.log(chalk.red('没有读取到路由文件 %s ，空转。'), configPath);
            originRouterMap = {};
        }
        return originRouterMap;
    },
    ...
    getServerConf() {
        ...
        try {
            // 读取站点路由配置
            fs.statSync(configPath);
            serverConf = require(configPath).genConf[env];
        } catch (err) {
            // 没有的话空转
            console.log(chalk.red('没有读取到服务server文件 %s ，空转。'), configPath);
            serverConf = {
                views: {
    ...
    getStaticRouterMap() {
        ...
        try {
            ...
            // 没有的话空转
            console.log(chalk.red('没有读取到静态路由文件 %s ，空转。'), configPath);
            staticRouterMap = {};
        }
        return staticRouterMap;
    },
