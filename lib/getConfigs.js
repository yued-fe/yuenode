...
// 缓存读取结果以提高性能
let siteConf, routerMap, serverConf, extendsLoader, staticRouterMap, IP;
...
module.exports = {
    ...
    getRouterMap() {
        ...
        routerMap = require(path.join(siteConf.path, siteConf.routermap_file));

        // 检查routerMap的数据格式，补全
        let parsedRoutes = {};

        for (let [k, v] of Object.entries(routerMap)) {
            let reqPath = '';
            let domain = '_';
            let pos = k.indexOf('/');
