...
module.exports = {
    ...
    requestCgi: function* (url, headers, options) {
        ...
        try {
            console.log(chalk.blue('Send request to backend:'), opt.uri);
            const startTime = Date.now();
            result = yield request(opt);
            spendTime = (Date.now() - startTime) / 1000;
            console.log(chalk.blue('Got backend response:'), result.statusCode, chalk.gray(spendTime + 's'));
        } catch (err) {
            err.message = 'Request backend failed: ' + err.message;
            throw err;
        }
    ...
    compressHTML(html){
        ...
        if (!!siteConf.compressHTML) {
            ...
            try {
                ...
                html += '<!-- min -->';
                console.log(chalk.red('HTML minify failed: \n'), err);
            }
        }
        return html;
    }
