...
// 获得自定义handler配置
exports.getCustomHandler = function() {
    var siteConf = getSiteConf();
    var customHandler;
    var custom_handle_file = siteConf['custom_handle_file'] || siteConf['NODE_SITE'];
    if (siteConf.custom_handle_on === 'true') {
        var path = siteConf['path'] + "/" + custom_handle_file;
        return require(path);
    }else{
        return require('../handler/default');
    }
}
